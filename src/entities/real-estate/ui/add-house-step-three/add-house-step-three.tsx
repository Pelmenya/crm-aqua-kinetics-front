...
import { getRealEstateLocation, getRealEstateState, getWaterIntakePointCount } from "../../model/real-estate-selectors";
import { decrementWaterIntakePoint, incrementWaterIntakePoint, setInitialState, setProgress, TRealEstateState } from "../../model/real-estate-slice";
...
export const AddHouseStepThree: FC = () => {
    ...
    // Получем локацию страницы от которой начали создавать объект
    ...
    const handleSave = useCallback(async () => {
        try {
            const newRealEstateData = await createRealEstate({
                ...
                authKey: authKey || '',
            }).unwrap() as TRealEstateState & {id: string };
            dispatch(setInitialState());
            dispatch(setSelectedRealEstateId(Number(newRealEstateData.id)))
