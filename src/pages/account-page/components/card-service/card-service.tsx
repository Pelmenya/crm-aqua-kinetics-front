...
export const CardService: FC<TCardServiceProps> = ({ title, status, children, fullWidth = false }) => {
    return (
        <Base>
            <div className={cn("flex gap-2 items-center", { "w-full justify-start pl-6": fullWidth })}>
                ...
                <div className="flex flex-col">
                    ...
                    {status ? <p className="text-min opacity-50 max-w-68 line-clamp-1">{status}</p> : null}
                </div>
            </div>
        </Base>
    )
